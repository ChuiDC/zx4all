...
// #define TRANS_COLOR2 0xFFFFFFFF
#define TRANS_COLOR2 0x33333333
...
SDL_Surface *text_screen=NULL, *text_image, *text_image2, *text_mac_upper_left, *text_mac_upper_right, *text_mac_upper, *text_mac_left, *text_mac_right, *text_mac_bottom_right, *text_mac_bottom_left, *text_mac_bottom, *text_background, *text_window_background;
...
//		text_screen=screen;
		...
		if (text_screen==NULL || tmp==NULL)
			exit(-1);
		text_image2=SDL_ConvertSurface(tmp,screen->format,SDL_RLEACCEL);
		SDL_FreeSurface(tmp);
		if (text_image2==NULL)
			exit(-2);
		...
		if (tmp==NULL)
			exit(-1);
		text_image=SDL_ConvertSurface(tmp,screen->format,SDL_RLEACCEL);
		SDL_FreeSurface(tmp);
		if (text_image==NULL)
			exit(-2);
		...
		if (tmp==NULL)
			exit(-3);
		text_mac_upper_left=SDL_ConvertSurface(tmp,screen->format,SDL_RLEACCEL);
		SDL_FreeSurface(tmp);
		...
		if (tmp==NULL)
			exit(-3);
		text_mac_upper_right=SDL_ConvertSurface(tmp,screen->format,SDL_RLEACCEL);
		SDL_FreeSurface(tmp);
		...
		if (tmp==NULL)
			exit(-3);
		text_mac_upper=SDL_ConvertSurface(tmp,screen->format,SDL_RLEACCEL);
		SDL_FreeSurface(tmp);
		...
		if (tmp==NULL)
			exit(-3);
		text_mac_left=SDL_ConvertSurface(tmp,screen->format,SDL_RLEACCEL);
		SDL_FreeSurface(tmp);
		...
		if (tmp==NULL)
			exit(-3);
		text_mac_right=SDL_ConvertSurface(tmp,screen->format,SDL_RLEACCEL);
		SDL_FreeSurface(tmp);
		...
		if (tmp==NULL)
			exit(-3);
		text_mac_bottom_left=SDL_ConvertSurface(tmp,screen->format,SDL_RLEACCEL);
		SDL_FreeSurface(tmp);
		...
		if (tmp==NULL)
			exit(-3);
		text_mac_bottom_right=SDL_ConvertSurface(tmp,screen->format,SDL_RLEACCEL);
		SDL_FreeSurface(tmp);
		...
		if (tmp==NULL)
			exit(-3);
		text_mac_bottom=SDL_ConvertSurface(tmp,screen->format,SDL_RLEACCEL);
		SDL_FreeSurface(tmp);
		...
		if (tmp==NULL)
			exit(-3);
		text_background=SDL_ConvertSurface(tmp,screen->format,SDL_RLEACCEL);
		SDL_FreeSurface(tmp);
		if (text_background==NULL)
			exit(-3);

		tmp=IMG_Load(MENU_FILE_WINDOW);
		if (tmp==NULL)
			exit(-4);
		text_window_background=SDL_ConvertSurface(tmp,screen->format,SDL_RLEACCEL);
		SDL_FreeSurface(tmp);
		if (text_window_background==NULL)
			exit(-5);
		SDL_SetColorKey(text_window_background,(SDL_SRCCOLORKEY | SDL_RLEACCEL),SDL_MapRGB(text_image -> format, 255, 255, 255));
	}
	if (splash)
	{
		SDL_Event ev;
		...
		if (tmp==NULL)
			exit(-6);
		sur = SDL_ConvertSurface(tmp,screen->format,0);
		SDL_FreeSurface(tmp);
		...
		for(i=128;(i>-8)&&(!toexit);i-=8)
		{
			...
			while(SDL_PollEvent(&ev))
			{
				if (ev.type == SDL_VIDEORESIZE)
					TV_ResizeWindow(ev.resize.w, ev.resize.h);
				else
			      	toexit=1;
			}
		}
...
*/
	SFont_PutString(text_screen, x, y, str);
}
...
*/
	SFont_PutString(text_screen, x*8, y*8, str);
}
...
void text_draw_window(int x, int y, int w, int h, char *title)
{
	...
	for(i=0;i<4;i++)
		for(j=0;j<2;j++)
		{
			dest.x=x+i*text_window_background->w;
			dest.y=y+j*text_window_background->h;
