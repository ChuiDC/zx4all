...
static unsigned long long statitics_z80_opcodes[0x500];
static unsigned long long statitics_z80_num=0;

void debugZ80_PrintStatitics(void)
{
	double total=0.0;
	int n=0;
	int top[DEBUG_Z80_STATS];
	puts("\n---------------------");
	memset(top,-1,sizeof(int)*DEBUG_Z80_STATS);
	for(n=0;n<DEBUG_Z80_STATS;n++)
	{
		int i, bck=-1;
		unsigned long long max=0;
		for(i=0;i<0x500;i++)
		{
			unsigned long long d=statitics_z80_opcodes[i];
			if (d>=max)
			{
				int yet=0;
				for(yet=0;yet<DEBUG_Z80_STATS;yet++)
					if (top[yet]==i)
						break;

				if (yet==DEBUG_Z80_STATS)
				{
					max=d;
					bck=i;
				}
		...
		double partial=(((double)max)*100.0)/((double)statitics_z80_num);
		total+=partial;

		if (bck<0x100)
			printf("%5.2f%% -> '%s' (%.2X)\n", partial,Mnemonics[bck] ,bck);
		else
		if (bck<0x200)
			printf("%5.2f%% -> '%s' (CB %.2X)\n", partial,MnemonicsCB[bck&0xff],bck&0xff);
		else
		if (bck<0x300)
			printf("%5.2f%% -> '%s' (ED %.2X)\n", partial,MnemonicsED[bck&0xff],bck&0xff);
		else
		if (bck<0x400)
			printf("%5.2f%% -> '%s' (DD %.2X)\n", partial,MnemonicsXX[bck&0xff],bck&0xff);
		else
			printf("%5.2f%% -> '%s' (DD CB %.2X)\n", partial,MnemonicsXCB[bck&0xff],bck&0xff);
...
{
	unsigned short ind=RdZ80(_z80_get_PC())&0x00FF;
	switch(ind)
	{
    		case 0xCB: ind=0x100|(RdZ80(_z80_get_PC()+1)&0x00FF); break;
		case 0xED: ind=0x200|(RdZ80(_z80_get_PC()+1)&0x00FF); break;
		case 0xDD:
		case 0xFD:
			   if (RdZ80(_z80_get_PC()+1)!=0xCB)
				ind=0x300|(RdZ80(_z80_get_PC()+1)&0x00FF);
			   else
				ind=0x400|(RdZ80(_z80_get_PC()+2)&0x00FF);
			   break;
	}
	if (!statitics_z80_num)
		memset(statitics_z80_opcodes,0,0x500*sizeof(unsigned short));
	statitics_z80_opcodes[ind]++;
...
/** bytes disassembled.                                     **/
/*************************************************************/
static char R_DAsm[256], H_DAsm[32];
static int DAsm(char *S,unsigned short A)
{
  char C,*T,*P;
  ...
  {
    case 0xCB: B++;
	       sprintf(S,"%.2X ",RdZ80(B)); S+=3;
	       T=MnemonicsCB[RdZ80(B++)];
	       break;
    case 0xED: B++;
	       sprintf(S,"%.2X ",RdZ80(B)); S+=3;
	       T=MnemonicsED[RdZ80(B++)]; 
	       break;
    case 0xDD: B++;C='X';
	       sprintf(S,"%.2X ",RdZ80(B)); S+=3;
  ...
  if(P=strchr(T,'^'))
  {
    strncpy(R_DAsm,T,P-T);R_DAsm[P-T]='\0';
    sprintf(H_DAsm,"%02X",RdZ80(B++));
    strcat(R_DAsm,H_DAsm);strcat(R_DAsm,P+1);
  }
  else strcpy(R_DAsm,T);
  if(P=strchr(R_DAsm,'%')) *P=C;

  if(P=strchr(R_DAsm,'*'))
  {
    strncpy(S,R_DAsm,P-R_DAsm);S[P-R_DAsm]='\0';
    sprintf(H_DAsm,"%02X",RdZ80(B++));
    strcat(S,H_DAsm);strcat(S,P+1);
  }
  else
    if(P=strchr(R_DAsm,'@'))
    {
      ...
      {
        strncpy(S,R_DAsm,P-R_DAsm);S[P-R_DAsm]='\0';
        sprintf(H_DAsm,"%04X",RdZ80(B)+256*RdZ80(B+1));
        strcat(S,H_DAsm);strcat(S,P+1);
        B+=2;
      }
      else strcpy(S,R_DAsm);
...
/*************************************************************/
static char Flags_debugZ80[32] = "SZ.H.PNC";
static char S_debugZ80[128],T_debugZ80[10];
unsigned char debugZ80(int interactive)
{
  unsigned char J,I;

  for(J=0,I=_z80_get_F();J<8;J++,I<<=1) T_debugZ80[J]=I&0x80? Flags_debugZ80[J]:'.';
  T_debugZ80[8]=0;

  printf
  (
    "AF:%04X HL:%04X DE:%04X BC:%04X PC:%04X SP:%04X IX:%04X IY:%04X R:%.04X\n",
    _z80_get_AF()&MASCARA_AF,_z80_get_HL(),_z80_get_DE(),_z80_get_BC(),
    _z80_get_PC(),_z80_get_SP(),_z80_get_IX(),_z80_get_IY(),_z80_get_R()
  );
  DAsm(S_debugZ80,_z80_get_PC());
  printf
  ( 
    "PC: [%02X - %s]   FLAGS: [%s]   %s %s"
#ifdef DEBUG_Z80_CYCLING
    "\t(%i)"
#endif
    "\n\n",
    RdZ80(_z80_get_PC()),S_debugZ80,T_debugZ80,
#ifndef USE_RAZE
    procesador.IFF&0x04? "IM2":procesador.IFF&0x02? "IM1":"IM0",
    procesador.IFF&0x01? "EI":"DI"
#else
    (z80_get_reg(Z80_REG_IM)==1)? "IM1": (z80_get_reg(Z80_REG_IM)==2)? "IM2": "IM0",
    z80_get_reg(Z80_REG_IFF1)? "EI":"DI"
#endif
#ifdef DEBUG_Z80_CYCLING
    ,_z80_get_last_cycles()
#endif
  );

#ifdef USE_MASCARA_AF
  if (RdZ80(_z80_get_PC())==0xF5) // PUSH AF
	  pushaf_found=2;
#endif
