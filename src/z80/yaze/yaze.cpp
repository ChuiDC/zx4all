...
unsigned short yaze_af[2]={0, 0};
int yaze_af_sel=0;
yaze_ddregs yaze_regs[2]= { {0, 0, 0}, {0, 0, 0} };
int yaze_regs_sel=0;
unsigned short yaze_ir=0;
unsigned short yaze_ix=0;
unsigned short yaze_iy=0;
unsigned short yaze_sp=0;
unsigned short yaze_pc=0;
unsigned short yaze_IFF=0;
...
#define LOAD_STATE()							\
    PC = yaze_pc;								\
    AF = yaze_af[yaze_af_sel];							\
    BC = yaze_regs[yaze_regs_sel].bc;						\
    DE = yaze_regs[yaze_regs_sel].de;						\
    HL = yaze_regs[yaze_regs_sel].hl;						\
    IX = yaze_ix;								\
    IY = yaze_iy;								\
    SP = yaze_sp
...
#define DECLARE_STATE()							\
    FASTREG PC = yaze_pc;							\
    FASTREG AF = yaze_af[yaze_af_sel];						\
    FASTREG BC = yaze_regs[yaze_regs_sel].bc;					\
    FASTREG DE = yaze_regs[yaze_regs_sel].de;					\
    FASTREG HL = yaze_regs[yaze_regs_sel].hl;					\
    FASTREG IX = yaze_ix;							\
    FASTREG IY = yaze_iy;							\
    FASTREG SP = yaze_sp
...
#define SAVE_STATE()							\
    yaze_pc = PC;								\
    yaze_af[yaze_af_sel] = AF;							\
    yaze_regs[yaze_regs_sel].bc = BC;						\
    yaze_regs[yaze_regs_sel].de = DE;						\
    yaze_regs[yaze_regs_sel].hl = HL;						\
    yaze_ix = IX;								\
    yaze_iy = IY;								\
    yaze_sp = SP


unsigned yaze_tstates=0;
...
unsigned yaze_emulate(unsigned ncycles) {
    FASTREG PC = yaze_pc;
    FASTREG AF = yaze_af[yaze_af_sel];
    FASTREG BC = yaze_regs[yaze_regs_sel].bc;
    FASTREG DE = yaze_regs[yaze_regs_sel].de;
	...
	case 0xED:			/* ED prefyaze_ix */
		...
		case 0x56:			/* IM 1 */
			...
			break;
		case 0x57:			/* LD A,I */
			AF = (AF & 0x29) | (yaze_ir & ~255) | ((yaze_ir >> 8) & 0x80) | (((yaze_ir & ~255) == 0) << 6) | ((yaze_IFF & 2) << 1);
			break;
		case 0x58:			/* IN E,(C) */
			temp = Input(lreg(BC));
			Setlreg(DE, temp);
		...
		case 0x5E:			/* IM 2 */
			...
			break;
		case 0x5F:			/* LD A,R */
			AF = (AF & 0x29) | ((yaze_ir & 255) << 8) | (yaze_ir & 0x80) | (((yaze_ir & 255) == 0) << 6) | ((yaze_IFF & 2) << 1);
			break;
		case 0x60:			/* IN H,(C) */
			temp = Input(lreg(BC));
			Sethreg(HL, temp);
